...
export class MoviesService {
    ...
    async list(queryParam) {
        try {
            ...
            if (queryParam && queryParam.q) {
                ...
                condition['$or'] = [{ name: new RegExp(queryParam.q, 'i') }, { genre: new RegExp(queryParam.q, 'i') }]
            }
    ...
    async save(movie) {
        try {
            ...
            const resultData = await this.movieModel.countDocuments({ name: movie.name, is_deleted: false }).exec();
            if (resultData)
                return { status: false, message: `${movie.name} movie is already exist` }
            const createdMoive = new this.movieModel(movie);
            await createdMoive.save();
            return { status: true, message: `${movie.name} added successfully to movies list` }
        } catch (e) {
    ...
    async update(param, movie) {
        try {
            ...
            const updateMoiveRes = await this.movieModel.updateOne({ _id: new mongoose.Types.ObjectId(param.id) }, movie).exec();
            console.log(updateMoiveRes)
            if (updateMoiveRes.modifiedCount)
                return true
            return false
        }
        catch (e) {
    ...
    async delete(movie) {
        try {
            const deleteMoive = this.movieModel.updateOne({ _id: new mongoose.Types.ObjectId(movie.id) }, { is_deleted: true, updated_date_time: new Date() });
